...
describe('runInitializationFlows', () => {
  ...
  const flowA = 'create-list-indices' as InitializationFlowId;
  ...
  it('runs multiple flows in parallel and returns all results', async () => {
    const { createListIndicesInitializationFlow } = jest.requireMock('./flows/create_list_indices');
    const nonExistingFlow = 'non-existing-flow' as InitializationFlowId;

    createListIndicesInitializationFlow.provision.mockResolvedValueOnce({ status: 'ready' });
