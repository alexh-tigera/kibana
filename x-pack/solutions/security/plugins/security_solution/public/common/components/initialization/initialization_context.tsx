  ...
  const requestInitialization = useCallback(
    async (flows: InitializationFlowId[]) => {
      const newFlows = flows.filter((id) => {
        ...
        // Skip flows that completed successfully (loading settled to false with no error).
        const flowState = stateRef.current[id];
        if (flowState && !flowState.loading && flowState.error == null) {
          return false;
        }
        return true;
      });
      ...
      try {
        ...
        retryableFlows = newFlows.filter((id) => {
          if (response.flows[id]?.status === 'ready') return false;
        ...
        setState((prev) => {
          ...
          for (const id of newFlows) {
            ...
            if (flowResult?.status === 'ready') {
              next[id] = { loading: false, result: flowResult.payload ?? null, error: null };
            } else if (retryableFlows.includes(id)) {
              next[id] = { loading: true, result: null, error: null };
            } else {
              next[id] = {
                loading: false,
        ...
        setState((prev) => {
          const next = { ...prev };
          for (const id of newFlows) {
            next[id] = retryableFlows.includes(id)
              ? { loading: true, result: null, error: null }
              : { loading: false, result: null, error: errorMessage };
          }
          return next;
        });
      }
