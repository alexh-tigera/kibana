...
export const openAIAdapter: InferenceConnectorAdapter = {
  chatComplete: ({
    ...
    // Some OpenAI-compatible gateways reject requests that contain tool-related messages
    // (assistant messages with tool_calls, or role:tool messages)
    // when no `tools` param is provided. This happens in multi-agent flows where a research agent
    // uses tools but the answering agent does not. Sanitize messages to convert tool interactions
    // into plain text when no tools are defined for the current request.
