...
const getIndexStatusByName = (
  ...
  indexNames.forEach((indexName) => {
    const { status } = indices.find((idx) => idx.name === indexName) ?? {};
    indexStatusByName[indexName] = status;
  });
  return indexStatusByName;
};

export interface UseManageIndexButtonHandlersParams {
  index: Index;
...
export function useManageIndexButtonHandlers({
  index,
  reloadIndexDetails,
  navigateToIndicesList,
  onIndexRefresh,
}: UseManageIndexButtonHandlersParams) {
  ...
  const indices = useMemo(() => [index], [index]);
  const indexStatusByName = useMemo(
    () => getIndexStatusByName(indexNames, indices),
    [indexNames, indices]
  );
