...
import { getSchemaAtPath } from '../../utils/zod';
...
export interface FormatZodErrorResult {
  message: string;
...
function generateUnionErrorMessage(
  ...
  try {
    ...
    const connectorUnionSchema = getConnectorUnionSchemaFromPath(path, yamlDocument, options);
    ...
    if (!result) {
      result = getBetterFieldErrorMessage(path, fieldName, yamlDocument, options);
    }
...
function getBetterFieldErrorMessage(
  ...
  try {
    ...
    if (
      ...
      const connectorSchema = getConnectorParamsSchema(stepType, options);
      if (!connectorSchema) {
        return null;
      }
...
function analyzeUnionSchema(
  ...
  const unionOptions: Array<{ name: string; description: string }> = [];
  ...
  for (const option of unionSchema._def.options) {
    ...
    unionOptions.push({ name, description });
  }
...
function findDiscriminatorInShape(
  ...
  for (const [key, schemaItem] of Object.entries(shape)) {
    if (schemaItem instanceof z.ZodLiteral) {
      // TODO: fix multiple values case
      return { key, value: schemaItem.value };
    }
  }
  return null;
}
 ...
 */
function isOptionalSchema(schemaItem: z.ZodType): boolean {
  return (
    schemaItem instanceof z.ZodOptional ||
    schemaItem instanceof z.ZodNullable ||
    schemaItem instanceof z.ZodDefault
  );
}
 ...
 */
function getSchemaTypeName(schemaItem: z.ZodType): string | null {
  try {
    return getCompactTypeDescription(schemaItem);
  } catch {
    return null;
  }
}
...
function getDynamicUnionErrorMessage(issue: any): string | null {
  ...
  const optionDescriptions = unionOptions.map((option) => `  - ${option.description}`).join('\n');
...
function analyzeUnionErrorForOption(issues: any[]): { name: string; description: string } | null {
  ...
  for (const issue of issues) {
    if (issue.code === 'invalid_literal' && issue.path && issue.path.length > 0) {
      const issueName = issue.path[issue.path.length - 1];
      const expectedValue = issue.expected;
      discriminatorInfo = { key: issueName, value: expectedValue };
    } else if (issue.code === 'invalid_type' && issue.path && issue.path.length > 0) {
      const issueName = issue.path[issue.path.length - 1];
      if (!requiredFields.includes(issueName)) {
        requiredFields.push(issueName);
      }
    }
  }
...
function getConnectorUnionSchemaFromPath(
  ...
  try {
    ...
    if (
      ...
      // Extract the field schema from the connector params schema
      const fieldSchema = getFieldSchemaFromConnectorParams(connectorSchema, String(stepFieldName));
      if (!fieldSchema) {
        return null;
      }
 ...
 * Requires a getConnectors callback to resolve connector definitions.
 * When no callback is provided, returns null (graceful fallback to generic messages).
 */
function getConnectorParamsSchema(
  ...
  try {
    const getConnectors = options?.getConnectors;
    if (!getConnectors) {
      return null;
    }
 ...
 */
function unwrapToUnion(schemaItem: z.ZodType): z.ZodUnion<any> | null {
