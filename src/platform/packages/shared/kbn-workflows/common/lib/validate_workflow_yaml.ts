...
import { parseWorkflowYamlToJSON } from './yaml/parse_workflow_yaml_to_json';
import type { ConnectorContractUnion, ValidateWorkflowResponseDto, WorkflowYaml } from '../..';
import type { WorkflowDiagnostic } from '../../types/v1';
...
export function validateWorkflowYaml(
  ...
  const parseOptions = options?.getConnectors
    ? { getConnectors: options.getConnectors }
    : undefined;
  const parseResult = parseWorkflowYamlToJSON(yaml, zodSchema, parseOptions);
