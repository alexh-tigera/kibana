...
import type { FormatZodErrorOptions } from '../zod/format_zod_error';
...
export type ParseWorkflowYamlToJSONOptions = FormatZodErrorOptions;
...
export function parseWorkflowYamlToJSON<T extends ZodType>(
  ...
  if (!result.success) {
    ...
    const { message, formattedError } = formatZodError(filteredError, schema, document, options);
    return {
      success: false,
